...
module tb_dbg_thr;
    ...
    alg_top #(
        .W(W), .VEXT(VEXT), .H(H), .REXT(REXT), .HALF(HALF), .AW(AW)
    ) u_top (
        .clk(clk), .rst_n(rst_n),
    ...
    integer fgray, fmed, fgau, fsob, fnms, fthr, fdsp, fdisp, fin, fvs, fdbg;
    integer frame = 0;
    reg vs_d = 0, o_vs_d = 0;
    reg [31:0] tick = 0;
    ...
    always @(posedge clk) begin
        ...
        if (o_vs & ~o_vs_d)
            $fwrite(fvs, "%0d\n", tick);
        if (o_de)
            $fwrite(fdisp, "%0d %02x%02x%02x\n", tick, o_r, o_g, o_b);
        // --- thr internal debug (tick, in: de/x/y, d-path: de/x/y/weak/mag, w-path: de/x/y/dil, out) ---
        $fwrite(fdbg, "%0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d %0d\n",
                tick, u_top.thr_din, u_top.thr_x, u_top.thr_y,
                u_top.u_thr.d_de, u_top.u_thr.d_x, u_top.u_thr.d_y,
                u_top.u_thr.weak_d, u_top.u_thr.mag_d,
                u_top.u_thr.w_de, u_top.u_thr.w_x, u_top.u_thr.w_y, |u_top.u_thr.w_dil);
    end
