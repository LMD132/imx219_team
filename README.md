...
- **已上板验证（数值层面）**：`uart-bringup` 分支的按键实时调阈值 + UART 遥测，以及 3×3 邻域计数去噪 + 目标外接框 + 帧像素计数（`candidate_bitstreams/overlay_box.bit`）。串口报文能证明设计在跑、按键动作逐次命中，且每帧 `PIX=921600`（= 1280 x 720，帧级无丢/无多像素），但**画面观感还没人描述过**，所以视觉上仍算未验收。见 `docs/key_threshold_control.md`、`docs/edge_overlay.md`。
...
| 3×3 邻域计数去噪 + 目标外接框 + 帧像素计数（赛题高阶 ⑥） | `overlay_box.bit` 已编译/下载，每帧 `PIX=921600` 证明帧级无丢像素；画面观感待确认，框的适用局限见 `docs/edge_overlay.md` |
