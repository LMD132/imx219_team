...
> **另见**：`docs/github_survey.md`（2026-09-26）是 GitHub 上的**广度调研**——15 组关键词、
> 67 条结果、8 个候选仓库的真实文件清单。三条要点：① 同平台（Ti60F225）存在 MIT 许可的
> `Floatkyun/Ultra-Vision`（113★，2024 全国赛国一 + 易灵思创新杯），其 HDMI TX 三件套与
> 插值/DDR 写法值得逐模块对照；② 无 license 的仓库一律只读思路；③ 参照视频（B 站
> 晓马哥FPGA）的源码**不在 GitHub**（简介指向 QQ 群分发），无法公开获取。
